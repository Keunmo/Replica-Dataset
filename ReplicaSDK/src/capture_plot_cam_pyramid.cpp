...
struct FloatingHandler : pangolin::Handler {
  FloatingHandler(pangolin::OpenGlRenderState &render_state): 
...
int main(int argc, char* argv[]) {
  ...
  // std::ofstream camPoseFile(outFolder + "/camPose.txt");
  ...
  while (!pangolin::ShouldQuit()) {
    // debug
    // std::cout << "DEBUG: start frame " << frameCnt << std::endl;

    glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT);

    // if (camPoses.size() > 0) {
    //   // std::cout << "DEBUG: camPoses size: " << camPoses.size() << std::endl;
    //   for (size_t i = 0; i < camPoses.size(); i++){
    //     DrawCurrentCamera(camPoses[i]);
    //   }
    // }
    ...
    if (meshView.IsShown()) {
      ...
      if (pangolin::Pushed(captureBtn)){
        ...
        // flip image
        const int channels = 3; // assuming RGB format
        const int rowSize = width * channels;
        std::vector<uint8_t> flippedImage(image.h * rowSize);
        for (size_t i = 0; i < image.h; ++i) {
            memcpy(&flippedImage[(image.h - 1 - i) * rowSize], &image.ptr[i * width], rowSize*sizeof(uint8_t));
        }
        memcpy(image.begin(), flippedImage.data(), flippedImage.size()*sizeof(uint8_t));
        // save image
        char filename[1000];
        snprintf(filename, 1000, "%s/images/frame%06d.jpg", outFolder.c_str(), frameCnt);
        pangolin::SaveImage(
        ...
        // write campose
        // Eigen::Matrix4d world2camPose = s_cam.GetModelViewMatrix();  // world to camera.
        ...
        if (renderDepth) {
          ...
          ptexMesh.RenderDepth(s_cam, 65535.0f * 0.1f);  // depthScale
          ...
          // flip image
          const int channels = 1; // assuming GRAY format
          const int rowSize = width * channels;
          std::vector<float> flippedImage(depthImage.h * rowSize);
          for (int i = 0; i < height; ++i) {
              memcpy(&flippedImage[(depthImage.h - 1 - i) * rowSize], &depthImage.ptr[i * rowSize], rowSize*sizeof(float));
          }
          memcpy(depthImage.begin(), flippedImage.data(), flippedImage.size()*sizeof(float));

          // convert to 16-bit int
          for(size_t i = 0; i < depthImage.Area(); i++)
              depthImageInt[i] = static_cast<uint16_t>(depthImage[i] + 0.5f);

          snprintf(filename, 1000, "%s/depths/depth%06d.png", outFolder.c_str(), frameCnt);
          pangolin::SaveImage(
